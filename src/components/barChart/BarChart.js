...
class BarChart extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.selectedCategory !== this.props.selectedCategory) {
      const res = this.props.costs.filter(
        elem => elem.product.category.name === this.props.selectedCategory,
      );
      const finalyResult = res.reduce((arr, el) => {
        const flag = arr.some(element => el.product.name === element.name);
        !flag &&
          arr.push({
            name: el.product.name,
            amount: res.reduce((acc, elem) => {
              elem.product.name === el.product.name && (acc += elem.amount);
              return acc;
            }, 0),
          });
        return arr;
      ...
      this.setState({
        innerData: sortData(finalyResult),
        isRender: true,
      });
    }
